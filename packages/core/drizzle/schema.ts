...
export const thread = pgTable(
	'Thread',
	{
		...
		starredAt: timestamp({ precision: 3, mode: 'date', withTimezone: true }),
		...timestamps,
	...
	(table) => [
		...
		index('Thread_lastSentAt_starredAt_idx').using(
			'btree',
			table.lastSentAt.desc().nullsFirst().op('timestamptz_ops'),
			table.starredAt.asc().nullsLast().op('timestamptz_ops'),
		),
