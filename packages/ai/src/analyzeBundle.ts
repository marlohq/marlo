...
function getMultiBundlePrompt(account: Account) {
	return `
# Instructions
...
export async function analyzeBundleSummary(
	threadData: string[],
	category: string,
	...
	try {
		const result = await generateObject({
			...
			system: getMultiBundlePrompt(account),
			schema: z.object({
			...
			messages: [
				{
					...
					content: [
						{
							type: 'text',
							text: `Grouped By: ${type}, ${category}\n\nEmails:\n${threadData.map((t) => `<email>${JSON.stringify(t)}</email>`).join('\n')}`,
						},
					],
				},
			],
		...
		const sanitizedError = new Error(
			`Failed to analyze bundle title for ${type} ${category}: ${error instanceof Error ? error.message : String(error)}`,
		);
